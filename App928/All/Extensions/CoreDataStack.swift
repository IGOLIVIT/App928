...
class CoreDataStack {
    ...
    func deleteServ(withServServ1 name: String, completion: @escaping () -> Void) {

        let context = CoreDataStack.shared.persistentContainer.viewContext
        let fetchRequest: NSFetchRequest<ServModel> = ServModel.fetchRequest()

        fetchRequest.predicate = NSPredicate(format: "servServ1 == %@", name)
    ...
    func deleteAp(withApServ1 name: String, completion: @escaping () -> Void) {

        let context = CoreDataStack.shared.persistentContainer.viewContext
        let fetchRequest: NSFetchRequest<AppModel> = AppModel.fetchRequest()

        fetchRequest.predicate = NSPredicate(format: "apServ1 == %@", name)
    ...
    func deleteDoc(withDocFunc name: String, completion: @escaping () -> Void) {

        let context = CoreDataStack.shared.persistentContainer.viewContext
        let fetchRequest: NSFetchRequest<DocModel> = DocModel.fetchRequest()

        fetchRequest.predicate = NSPredicate(format: "docFunc == %@", name)
